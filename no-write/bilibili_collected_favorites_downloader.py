...
# 用户我追的合集/收藏夹
## 读取data_users_collected_mid.json文件
### 获取用户mid
with open('data_users_collected_mid.json', 'r') as f:
    users_collected_mid_list = json.load(f)

## 检查用户mid是否为空
if not users_collected_mid_list:
    print("data_users_created_mid.json文件为空")
else:
## for循环每个用户的收藏夹URL
## 为每个mid构建收藏夹URL
    for users_collected_mid in users_collected_mid_list:
        favorites_url = f"https://api.bilibili.com/x/v3/fav/folder/collected/list?pn=1&ps=50&up_mid={users_collected_mid}&platform=web&web_location=333.1387"
        print(f"用户我追的合集/收藏夹URL:", favorites_url)
        ...
        for favorite in favorites_list:
            ...
            '''  这次的收藏夹和以前的不一样
            收藏夹的fid调用api：：
            完整链接https://api.bilibili.com/x/space/fav/season/list?season_id=1325566&pn=1&ps=40&web_location=333.1387
            花括号内为变量"<收藏夹fid,也是变量"favorite_id">"https://api.bilibili.com/x/space/fav/season/list?season_id={fid}&pn=1&ps=40&web_location=333.1387"
            ...
            # urls.append(url)
            ## 拼接收藏夹网址
            favorite_url = "https://api.bilibili.com/x/space/fav/season/list?season_id=" + favorite_id + "&pn=1&ps=40&web_location=333.1387"
